...
at <- readRDS(file.path(raw_dat, "rekn_atlantic_20231219.rds")) %>% 
  dplyr::select(-algorithm.marked.outlier, -import.marked.outlier) %>%
...
john <- readRDS(file.path(raw_dat, "rekn_johnson_raw_20231219.rds"))%>%
  mutate(tag.id = as.character(tag.id))
sth <- readRDS(file.path(raw_dat, "rekn_sthcarolina_20231219.rds")) %>%
  mutate(tag.id = as.character(tag.id))
spring <- readRDS(file.path(raw_dat, "rekn_spring_usfws_20231219.rds"))%>%
  mutate(tag.id = as.character(tag.id)) 
...
all <- bind_rows(at, eccc, ma, oc, qu, dom, mils, john, sth, spring, new)

# 
# 
# all <- all %>%
#   dplyr::select(-"algorithm.marked.outlier",
...
#length(all$visible) #103256

# write out: 
write.csv(all , file.path("output", "xall_rekn_20240119.csv"), row.names = F)
allsp = all %>% 
  filter(!is.na(location.long), 
         !is.na(location.lat))
clean_sf <- st_as_sf(allsp, coords = c("location.long", "location.lat"), crs = st_crs(4326))
st_write(clean_sf, file.path("output", "xall_rekn_20240119.gpkg"), append = F)
...
clean = all %>% 
  dplyr::select(-argos.altitude, -import.marked.outlier, -deploy.on.date, 
                ...
                -height.above.ellipsoid,-capture.timestamp,-deploy.on.latitude,
                -deploy.on.longitude, -deploy.on.person, -deployment.id, -deploy.on.date,
...
write.csv(clean, file.path("output", "xsubset_rekn_20240119.csv"))
...
st_write(clean_sf, file.path("output", "xclean_rekn_20240119.gpkg"), append = F)
