...
at <- readRDS(file.path(raw_dat, filesoi[4]))
eccc <- readRDS(file.path(raw_dat, filesoi[5]))
ma <- readRDS(file.path(raw_dat, filesoi[6]))
oc <- readRDS(file.path(raw_dat, filesoi[7]))


all <- rbind(at, eccc, ma, oc)
...
clean = all
#write.csv(clean, file.path("output", "all_rekn_20230918.csv"))
#saveRDS(clean_save, file = file.path("output", "all_rekn_20230918.rds"))
```

The number of cleaned records we have compiled is `r length(clean$tag.local.identifier)`, from 
`r length(unique(clean$tag.local.identifier))` individuals.
