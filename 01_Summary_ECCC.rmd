...
alldat <- left_join(dur_data, havdf, by = c("individual.local.identifier.x"))

```

Th e great circle distance was calculated using the rough first and last loaction points as a rough proxy for the total movement 
...
gcp_hist <- ggplot(alldat, aes(x= gcd))+
  geom_histogram(bins = 1000) + #fill="white", position="dodge") +
  #facet_wrap(~year)+
  xlab("distance (km)") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10))
gcp_hist
